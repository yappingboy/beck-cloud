...
### New Service: Documentation First (Mandatory)

**When you request a new service, Nova MUST write the `docs/services/<name>.md` file BEFORE creating any Flux manifests.** The doc is the design document. It states the intended behavior, the ports, the PVCs, the IngressRoute, the SSO chain, and the resources. The config must then match the doc. This prevents doc drift.

The workflow:

1. **Request** (you): name, purpose, SSO chain, port, PVC needs, image.
2. **Doc** (Nova): writes `docs/services/<name>.md` per [Writing-Standard.md](./Writing-Standard.md). States image, ports, PVCs, IngressRoute, SSO chain, resources. Runs `ste-check.py` until 0 FAIL.
3. **Config** (Nova): writes the Flux manifests. Every fact in the doc must be reflected in the YAML.
4. **Verify** (Nova): runs `ste-check.py` on the new doc. Verifies the config matches the doc (image, ports, PVCs, SSO, host).
5. **Commit** (Nova): doc + config in the same commit. Commit message references the service name.

If the request is ambiguous, Nova asks ONE clarifying question before writing the doc. If the doc and config diverge, the doc wins (it is the design).
