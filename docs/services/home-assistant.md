...
**What it does:** Home Assistant (`ghcr.io/home-assistant/home-assistant:stable`) runs as a containerized instance. It provides a web UI for controlling IoT devices, automations, and integrations. The pod also runs sidecar containers: Mosquitto (MQTT on 1883), ESPHome (dashboard on 6052), and Matter Server (5580). External access is at `ha.becklab.cloud` via Traefik. Configuration, user data, and device states persist on a volume.
