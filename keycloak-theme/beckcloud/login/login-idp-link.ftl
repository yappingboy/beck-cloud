...
<@layout.displayLayout>
  ...
  <div id="kc-header-wrapper">
    <div id="kc-header">${msg("loginIdpConfirmTitle", idpDisplayName!"")}</div>
    <div id="kc-header-subtitle">${msg("loginIdpLinkTitle")}</div>
  </div>

  <div id="kc-card">
    <div class="revoke-info">
      <p>${msg("loginIdpConfirmExisting")}</p>
    </div>

    <form id="kc-register-form" action="${url.loginAction}" method="post">
      <div class="form-group">
        <label for="username" class="form-label">${msg("loginIdpConfirmUsername")}</label>
        <input type="text" id="username" class="form-control" name="username"
               value="${(auth.username!'')}" autocomplete="username" autofocus/>
      </div>

      <div class="form-group">
        <label for="email" class="form-label">${msg("loginIdpConfirmEmail")}</label>
        <input type="email" id="email" class="form-control" name="email"
               value="${(auth.email!'')}" autocomplete="email"/>
      </div>

      <div id="kc-form-buttons">
        <button type="submit" id="kc-login" class="kc-btn kc-btn-primary">${msg("loginIdpConfirmSubmit")}</button>
      </div>
    </form>
